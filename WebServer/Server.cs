...
namespace Webserver
{
    ...
    public class Server
    {
        ...
        public Server(int portNumber = 80, bool DhcpEnable = false)
        {
            ...
            _ListeningSocket.SendTimeout = 10000;
            _ListeningSocket.ReceiveTimeout = 10000;
                    ...
                    using (Socket clientSocket = _ListeningSocket.Accept())
                    {
                        ...
                        do
                        {
                            if (availableBytes < clientSocket.Available)
                            {
                                ...
                                LoopCount += 1;
                            Thread.Sleep(1);
                        } while (availableBytes == 0 || LoopCount < 400);
                        ...
                        if (availableBytes < Settings.MAX_REQUESTSIZE && availableBytes > 0)
                        {
                            ...
                            {
                                Debug.Print("Final byte count: " + availableBytes + " | Request URL=" + tempRequest.URL);

                                if (tempRequest.Method == "POST")
                                {
                                    //POST was incoming, it will be saved to SD card at Settings.POST_TEMP_PATH
                                    PostToSdWriter post = new PostToSdWriter(tempRequest, buffer);
                                    post.Receive();
                                }
